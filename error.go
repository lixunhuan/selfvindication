...
func ErrorRec(level glog.Level, req *http.Request, logPayload bool, headers ...string) {
	v := glog.V(level)
	if v {
		bs := bytes.Buffer{}
...
func ErrorRes(level glog.Level, err error, resp *http.Response, decodingFunc func(res *http.Response) string, headers ...string) {
	v := glog.V(level)
	if v {
		if err != nil {
			v.Infof("%+v \n", err)
